# tp3-sistemas-operacionais

## Instruções

Para executar o xv6, execute os seguintes comandos no terminal:

1) `wsl` (Caso você esteja no Windows).
2) `make docker` ou `./run-docker.sh` (Montar o docker).
3) `make clean` (Opcional: Só se quiser limpar ou por precaução por causa da pasta mkfs).
4) `make run` (Ou então separadamente: `make` para compilar e `make qemu` para executar).

Para sair do xv6, use `Ctrl-A x` (`x` minúsculo).
